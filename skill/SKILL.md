...
1. If Aside already selected a relevant skill directly, search its exact frontmatter name to get its current path/hash; do not restart a broad inventory. Otherwise run `search --query "task terms"` with one to three useful terms against current metadata. If there is no lexical match, try synonyms or page `catalog --limit 20 --offset 0`; a lexical miss does not prove no useful skill exists. Check `status` for clearly relevant archived managed skills. If one is needed, run `restore --name "<managed name>"`, then discover and hash-load it normally.
...
4. Create the task scratch directory before writing files. Use exactly the normalized record shape below, adding only the optional `execution` object; `context` and `effect` belong inside `execution`, not at the top level. Write the record to task scratch. Use a stable nonprivate event identifier and preserve it on retries. Run `learn --record "<scratch record>"`, then `maintain --apply`. Read each result before claiming success. Under the installed managed-only policy, do not ask the user to approve each skill or routine archive.
...
2. For a matching managed skill, copy its exact registered name (including its single `oma-` prefix) and the context from `execution.json` or its generated Execution section into `route --name "<exact-name>" --context "<exact-context>"`. Never construct another prefix or invent a context from the request. Context describes the site/workflow/input shape, not a personal account identifier. Markdown-only, stale, unproven or failed routes fall back to the existing MD procedure. A recommendation is not execution success. On `missing-skill` or `context-mismatch`, check those exact identifiers once before concluding no saved route exists. If only `validation-needed` blocks a relevant read-only recipe, check applicability against its MD and use at most one `begin --probe` for the current requested work. Record it as candidate validation; do not run extra tasks just to fill counters. Otherwise follow the MD procedure.
...
This lifecycle is automatic only in sessions that honor this installed bootstrap. This is session guidance, not a global completion hook; direct selection of a managed skill must still enter this bootstrap. Lifecycle or finished session signals do not prove success. CLI input is normalized records only: never feed raw transcripts, credentials, or provider databases.
